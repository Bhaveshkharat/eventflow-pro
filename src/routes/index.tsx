...
import { events, plans, stats, testimonials } from "@/data/mock";
    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 pt-16 pb-32 text-center">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.25 }}
            ...
            <Link to="/register"><GradientButton size="lg">Start free <ArrowRight className="h-4 w-4" /></GradientButton></Link>
            <Link to="/events" className="px-6 py-3.5 rounded-full glass text-sm font-medium hover:scale-[1.03] transition-transform">Browse events</Link>
          </motion.div>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20">
        <div className="flex items-end justify-between gap-6 mb-10">
          ...
          <Link to="/events" className="hidden md:inline-flex text-sm font-medium gradient-text">View all →</Link>
        </div>
        ...
        <div className="mt-12 grid md:grid-cols-3 gap-5">
          {plans.map(p => (
            <GlassCard key={p.id} className={"p-6 relative " + (p.popular ? "ring-2 ring-primary/40" : "")}>
              ...
              <Link to="/pricing" className="mt-5 block text-sm font-medium gradient-text">See full plan →</Link>
            </GlassCard>
          ))}
        </div>
      </section>
        ...
        <div className="relative overflow-hidden rounded-3xl glass-strong p-10 md:p-16 text-center shadow-elegant">
          ...
          <div className="relative">
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-3">
              <Link to="/register"><GradientButton size="lg">Get started free <ArrowRight className="h-4 w-4" /></GradientButton></Link>
              <Link to="/pricing" className="px-6 py-3.5 rounded-full glass text-sm font-medium">View pricing</Link>
            </div>
          </div>
        </div>
      </section>
