...
import { getPersistedServices, CompanionService } from "@/lib/servicesStore";

export default function HotelTravel() {
  const [hotelsList, setHotelsList] = useState<any[]>([]);

  useEffect(() => {
    const services = getPersistedServices();
    const mapped = services
      .filter(s => s.category === "Hotel")
      .map(s => ({
        id: s.id,
        name: s.name,
        distance: s.distance || "Near venue",
        rating: s.rating,
  ...
  return (
    <DashboardShell title="Hotel & Travel" subtitle="Manage room blocks, partner inventory and traveler bookings.">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatCard icon={Hotel} label="Partner hotels" value={hotelsList.length} delta="+2" />
      ...
      <div className="mt-8 grid lg:grid-cols-3 gap-6">
        <GlassCard className="lg:col-span-2 p-6" hover={false}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            {hotelsList.map(h => <HotelCard key={h.id} hotel={h} />)}
          </div>
        </GlassCard>
            ...
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl hover:bg-accent/40 transition-colors">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{hotelsList[i % hotelsList.length]?.name || "Guest Hotel"}</p>
                </div>
